...
class FaceDetection:
    ...
    def __init__ (self):
        ...
        self.onnx_path = os.path.join(os.path.dirname(__file__), 'Model', 'ultra_light_640.onnx')
        self.onnx_model = onnx.load(self.onnx_path)
        self.predictor = prepare(self.onnx_model)
        self.ort_session = ort.InferenceSession(self.onnx_path)
        self.input_name = self.ort_session.get_inputs()[0].name

        self.dlibFacePredictor = os.path.join(os.path.dirname(__file__), 'Model', 'shape_predictor_68_face_landmarks.dat')
    ...
    def make_rectangle(self,frame):
        ...
        confidences, boxes = self.ort_session.run(None, {self.input_name: img})  # run onnx
    ...
    def aligner(self,box, raw_img):
        x1, y1, x2, y2 = box

        landmarks = self.align.findLandmarks(np.asarray(raw_img),
        ...
        aligned_face = self.align.align(96, raw_img,bb=dlib.rectangle(left=x1, top=y1, right=x2, bottom=y2), landmarks=landmarks,
    ...
    def authorize_user(self,train_images, test_image, distance=0.5, similarity=0.02):
        self.init()
        ...
        dists = []
        test_image_box = self.get_boxes(test_image)
        test_image = self.aligner(test_image_box[0],test_image)
        test_rep = self.getRep(test_image)
        for train_img in train_images:
            train_image_box = self.get_boxes(train_img)
            train_img = self.aligner(train_image_box[0],train_img)
            train_rep = self.getRep(train_img)
            dists.append(self.get_euclidian_distance(train_rep, test_rep))
    ...
    def get_dist_from_images(self,train_image, test_image, distance=0.5, similarity=0.02):
        if self.net is None:
            self.init()
        ...
        test_rep = self.getRep(test_image)
        train_rep = self.getRep(train_image)

        return self.get_euclidian_distance(train_rep, test_rep)


    def init(self):
        imgDim = 96
        #if self.net is None and platform.system() == 'Linux':
        self.net = openface.TorchNeuralNet(os.path.join(os.path.dirname(__file__), 'Model', 'nn4.small2.v1.t7'), imgDim)
