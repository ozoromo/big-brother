...
class Threshold_Calc:
    ...
    # Calculates the recall for a given threshold. Recall is "a models ability to find all datapoints of interest" : true positives / (true positives + false negatives)
    ...
    # Calculates precision for a given threshold. Precision is proportion of true positives in all the points that the model says are positive: true positives / (true positives + false positives)
    ...
    # Generates a list of thresholds bbetween [min_threshold, max_threshold] with {steps_num} steps between them. If no values are given, the min. and max. are calculated based on data in self.data
    ...
    def set_thres_range(self, min_threshold = 0, max_threshold = 0, step_num = 10, data = None):
        #if data == None:
            #data = self.data
        self.threshold_range = self.calc_threshold_range(min_threshold = min_threshold, max_threshold = max_threshold, step_num = step_num, data = data)
